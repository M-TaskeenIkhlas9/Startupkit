...
2. **Root Directory: `apps/api`** — `vercel.json` now lives inside that folder (deliberately not
   at the repo root — a root-level one gets read by *every* Vercel project on this repo, which
   silently broke the frontend project's Next.js build the first time around).
3. In the same Root Directory settings, turn **ON** "Include files outside the root directory in
   the Build Step" — this is what lets `apps/api/vercel.json`'s `includeFiles: ["../../src/**"]`
   reach up and pull in `src/startupkit`, which lives outside `apps/api`.
4. Framework Preset: **Other** (it's not a Next.js app; Vercel will use `vercel.json`).
5. **Attach Vercel KV** (Storage tab → Create → KV, or connect an existing one) — this is what
...
7. Deploy. Note the resulting URL (e.g. `https://startupkit-api.vercel.app`) — the frontend needs it.

## 2. Frontend project (`apps/web`)

1. Vercel → **Add New Project → Import** the same repo again (a second project).
2. **Root Directory: `apps/web`**.
3. Leave "Include files outside the root directory in the Build Step" **OFF** here — the frontend
   is self-contained, and turning it on would risk it discovering the backend's
   `apps/api/vercel.json` again (each project's include-outside-root setting only reaches into the
   repo, it doesn't limit *which* vercel.json a project can see, so keeping this off on the
   frontend is the actual isolation, not just an optimization).
4. Framework Preset: Next.js (auto-detected).
5. Environment Variables → `NEXT_PUBLIC_API_URL` = the backend URL from step 1.7.
6. Deploy.

## 3. Close the loop

Go back to the **backend** project → Environment Variables → set `CORS_ORIGINS` to the frontend's
real URL from step 2.6 → redeploy the backend (env var changes need a redeploy to take effect).
