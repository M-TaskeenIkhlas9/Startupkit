...
w1_formation = WorkflowManifest(
    ...
    entry="name-checks",
    steps={
        "name-checks": ActionStep(capability="incorporation", action="check_name_availability", next="name-decision"),
        "name-decision": DecisionStep(on="name_conflict", yes="name-checks", no="emit-name-confirmed"),
        # unblocks W4 + W5
        ...
        "sign-docs": HumanTaskStep(task="all-founders-sign", remind_after_hours=48, next="file-state"),
        "file-state": ActionStep(capability="incorporation", action="file_with_state", next="await-cert"),
        ...
        "file-83b": HumanTaskStep(task="file-83b-election", remind_after_hours=24, next="emit-formed"),
